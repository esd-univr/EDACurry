...
for i in range(1, len(sys.argv)):
    ...
    if os.path.isdir(argument):
        ...
        for filename in os.listdir(argument):
            ...
            out_eldo = os.path.join("spectre_result/", "{}.cir".format(name))
            out_spectre = os.path.join("spectre_result/", "{}.scs".format(name))
            # Get the content.
            print("Parsing `{}`".format(inp))

            start_time = time.time()
            root = edacurry.parse_spectre(inp)
            end_time = time.time()
            elapsed_time = end_time - start_time
            print(f"Parsing time: {elapsed_time:.4f} seconds")
    ...
    # If the argument is a file.
        ...
        end_time = time.time()
        elapsed_time = end_time - start_time
        print(f"Writing to XML time: {elapsed_time:.4f} seconds")
        # If required generate the output file.
        print("Writing `{}`".format(out_xml))
        with open(out_xml, "w") as outf:
            outf.write(xml_content)
        ...
        end_time = time.time()
        elapsed_time = end_time - start_time
        print(f"Writing to JSON time: {elapsed_time:.4f} seconds")
        # If required generate the output file.
        print("Writing `{}`".format(out_json))
        with open(out_json, "w") as outf:
            outf.write(json_content)
        ...
        end_time = time.time()
        elapsed_time = end_time - start_time
        print(f"Writing to Spectre time: {elapsed_time:.4f} seconds")
        # If required generate the output file.
        print("Writing `{}`".format(out_spectre))
        with open(out_spectre, "w") as outf:
            outf.write(spectre_content)
        ...
        end_time = time.time()
        elapsed_time = end_time - start_time
        print(f"Writing to Eldo time: {elapsed_time:.4f} seconds")
        # If required generate the output file.
        print("Writing `{}`".format(out_eldo))
        with open(out_eldo, "w") as outf:
            outf.write(eldo_content)
